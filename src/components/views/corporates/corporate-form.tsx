...
import { useGetCustomerGroups } from "../customer-groups/queries";
...
export function CorporateForm(props: Props) {
	...
	const { data, error: detailError } = useGetCorporateDetail({
		id: props.id || "",
		options: { enabled: !!props.id },
	});
	const { data: customerGroups, error: customerGroupsError } =
		useGetCustomerGroups({ page: 1, pageSize: 10 });
	useErrorHandler({
		error: createError || updateError || detailError || customerGroupsError,
	});
		...
		if (!disableSubmit) {
			const payload = {
				data: {
					...
					customer_group: +state.values.customerGroup,
				},
			};
			if (props.id) {
				updateFn(
			...
			<Box bgColor='#ffffff' borderRadius='10px'>
				...
				<form onSubmit={onSubmit}>
					<VStack p={4} alignItems='flex-start' spacing={4}>
						<FormControl isInvalid={Boolean(state.errors.customerGroup)}>
							<FormLabel>Customer Group</FormLabel>
							<Select
								name='customerGroup'
								placeholder='Select Customer Group'
								value={state.values.customerGroup}
								onChange={onChange}
							>
								{(customerGroups?.data || []).map((el) => (
									<React.Fragment key={el.id}>
										<option value={el.id}>{el.name}</option>
									</React.Fragment>
								))}
							</Select>
							{state.errors.customerGroup && (
								<FormErrorMessage>
									{state.errors.customerGroup}
								</FormErrorMessage>
							)}
