...
export function Corporates() {
	...
	const { data, refetch } = useGetCorporates({ page, pageSize });
	const { mutate: deleteFn, isLoading: isDeleting } = useDeleteCorporate();
	...
	return (
		<>
			<Box bgColor='#ffffff' borderRadius='10px'>
				...
				>
					<Text>Corporate List</Text>
					<Button
						as='a'
						href={URLS.CORPORATES_CREATE}
						variant='outline'
						colorScheme='blue'
					>
						Add Corporate
