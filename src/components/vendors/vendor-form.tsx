...
import { Fields, State, initialState, reducer } from "./vendor-form-reducer";
import {
	useCreateVendor,
	useGetVendorDetail,
	useUpdateVendor,
...
export function VendorForm(props: Props) {
	...
	const { mutate: createFn, isLoading: isCreating } = useCreateVendor();
	const { mutate: updateFn, isLoading: isUpdating } = useUpdateVendor();
	const { data } = useGetVendorDetail({
		id: props.id || "",
	...
	return (
		<>
			<Box bgColor='#ffffff' borderRadius='10px'>
				...
				>
					<Text>{props.id ? "Update" : "Create"} Vendor</Text>
				</HStack>
				<form onSubmit={onSubmit}>
					<VStack p={4} alignItems='flex-start' spacing={4}>
						...
						<HStack justifyContent='flex-end' w='full'>
							<Box>
								<Button as='a' variant='ghost' mr={3} href={URLS.VENDORS}>
									Cancel
								</Button>
								<Button
									type='submit'
