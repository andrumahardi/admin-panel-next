...
export function BankTransferForm(props: Props) {
	...
	const { mutate: createFn, isLoading: isCreating } = useCreateBankTransfer();
	const { mutate: updateFn, isLoading: isUpdating } = useUpdateBankTransfer();
	const { data } = useGetBankTransferDetail({
		id: props.id || "",
