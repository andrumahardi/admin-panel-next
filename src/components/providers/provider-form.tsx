...
import { Fields, State, initialState, reducer } from "./provider-form-reducer";
import {
	useCreateProvider,
	useGetProviderDetail,
	useUpdateProvider,
} from "./queries";
...
export function ProviderForm(props: Props) {
	const router = useRouter();

	const { mutate: createFn, isLoading: isCreating } = useCreateProvider();
	const { mutate: updateFn, isLoading: isUpdating } = useUpdateProvider();
	const { data } = useGetProviderDetail({
		id: props.id || "",
	...
	return (
		<>
			<Box bgColor='#ffffff' borderRadius='10px'>
				...
				>
					<Text>{props.id ? "Update" : "Create"} Provider</Text>
				</HStack>
					...
					<VStack p={4} alignItems='flex-start' spacing={4}>
						<FormControl isInvalid={Boolean(state.errors.code)}>
							...
							<Input
								...
								onChange={onChange}
								placeholder='Input Provider Code'
							/>
							{state.errors.code && (
						...
						<HStack justifyContent='flex-end' w='full'>
							<Box>
								<Button as='a' variant='ghost' mr={3} href={URLS.PROVIDERS}>
									Cancel
								</Button>
								<Button
									type='submit'
